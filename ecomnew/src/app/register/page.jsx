...
import { Car, Eye, EyeOff, ArrowRight, CheckCircle, Store } from "lucide-react";
        ...
        <div className="min-h-screen flex items-center justify-center px-4 py-12 bg-white">
            ...
            <div className="w-full max-w-md relative z-10 animate-fade-in">
                <div className="text-center mb-8">
                    ...
                    <p className="text-sm mt-1" style={{ color: "var(--text-muted)" }}>Start shopping quality auto parts</p>
                </div>
                ...
                <div className="card">
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="tel" className="input-field" placeholder="+256 700 000 000" value={form.phone} onChange={e => setForm({ ...form, phone: e.target.value })} />
                        </div>
                        <div>
                            <label className="block text-sm font-medium mb-2" style={{ color: "var(--text-secondary)" }}>Password</label>
                            <div className="relative">
                                <input type={showPassword ? "text" : "password"} required minLength={8} className="input-field pr-10" placeholder="Min 8 characters" value={form.password} onChange={e => setForm({ ...form, password: e.target.value })} />
                        ...
                        <button type="submit" disabled={loading} className="btn-primary w-full flex items-center justify-center gap-2">
                            {loading ? <span className="spinner w-4 h-4" /> : <><span>Create Account</span><ArrowRight size={16} /></>}
                        </button>
                    </form>
                    ...
                    <p className="text-center text-sm" style={{ color: "var(--text-muted)" }}>
                        ...
                        <Link href="/login" className="font-semibold hover:underline text-primary-dark">Sign in</Link>
                    </p>
                    <p className="text-center text-sm mt-3 pt-3 border-t border-slate-100" style={{ color: "var(--text-muted)" }}>
                        Looking to sell?{" "}
                        <Link href="/vendor" className="font-bold text-primary-dark hover:underline">Join Phamon Network</Link>
                    </p>
                </div>
            </div>
        </div>
